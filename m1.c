...
//Eso es lo que se implementa ahora:
    ...
    while((entry=readdir(proc))!=NULL){//Iterando pro cada emmtrada del dir 
        if(entry->d_type==DT_DIR){//viendo a ver si es DT_DIR
            //si si es un dir enonces segido de eso va un pid, si ese pid esta corriendo el entry->d_name va a ser != 0 entonces tiene memoria virtual en uso 
            // o sea si el PID ya termino eso quiere decir que no usa memoria virtual, por ahi va la logica de este loop
            int pid =atoi(entry->d_name);
            if(pid!=0){//si el name es != 0 entonces 
                Tmv_too_add = mv_pid(pid);
                    if (Tmv_too_add != -1) {
                        Tmv=Tmv+Tmv_too_add;
                    }
            }
...
int main() {
    ...
    printf("------------------------------------------------------------------\n\n");
    ////////////////////////////////////////ABRE////////////////////////
    DIR *proc_dir = opendir("/proc");// este dir solo se usa para filtrar los PID que existen en proc los que estan corriendo
    struct dirent *entry; //se usa para reprecentar el dir
